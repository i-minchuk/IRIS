...
class Document(Base):
    ...
    crs_approved_by: Mapped[Optional[str]] = mapped_column(
    ...
    planned_ready: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    actual_ready: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    checker_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("users.id"),
        nullable=True,
    )
...
class Revision(Base):
    ...
    )  # customer_comment, internal_review, design_change, error_fix, regulation_update, crs_response
    ...
    change_sheet: Mapped[Optional["ChangeSheet"]] = relationship(
...
class ApprovalWorkflow(Base):
    ...
    )  # in_progress, completed, rejected, escalated
...
class ApprovalStage(Base):
    ...
    stage_id: Mapped[str] = mapped_column(
        String(50)
    )  # author_check, peer_review, project_lead, tech_editor, etc.
