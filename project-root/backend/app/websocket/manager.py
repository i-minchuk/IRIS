...
class ConnectionManager:
    ...
    async def _redis_broadcast_handler(self, payload: dict):
        """Handle broadcast messages received from Redis."""
        message = payload.get("message", {})
        exclude_user_id = payload.get("exclude_user_id")
        await self._local_broadcast(message, exclude_user_id=exclude_user_id)

    async def _redis_document_broadcast_handler(self, payload: dict):
        """Handle document broadcast messages received from Redis."""
        document_id = payload.get("document_id")
        message = payload.get("message", {})
        exclude_user_id = payload.get("exclude_user_id")
        await self._local_broadcast_to_document(
            document_id, message, exclude_user_id=exclude_user_id
        )
    ...
    async def connect(self, user_id: int, websocket: WebSocket) -> None:
        ...
        if user_id in self.connections:
            ...
            try:
                await self.connections[user_id].close(
                    code=1008, reason="Multiple connections not allowed"
                )
            except Exception:
                pass
        ...
        # Subscribe to Redis Pub/Sub channels for this instance
        redis_pubsub.subscribe("ws:broadcast", self._redis_broadcast_handler)
        redis_pubsub.subscribe(
            "ws:document_broadcast", self._redis_document_broadcast_handler
        )
        ...
        while user_id in self.connections:
            try:
                ...
                if missed_pings >= self.MAX_MISSED_PINGS:
                    logger.warning(
                        f"User {user_id} missed {missed_pings} pings, disconnecting"
                    )
    ...
    async def _local_broadcast(
        self, message: dict, exclude_user_id: Optional[int] = None
    ) -> None:
    ...
    async def broadcast(self, message: dict, exclude_user_id: Optional[int] = None) -> None:
        """Broadcast message to all users (local + via Redis)."""
        await self._local_broadcast(message, exclude_user_id=exclude_user_id)
        await redis_pubsub.publish(
            "ws:broadcast",
            {"message": message, "exclude_user_id": exclude_user_id},
        )

    async def _local_broadcast_to_document(
        self,
        document_id: int,
        message: dict,
        exclude_user_id: Optional[int] = None,
    ) -> None:
        """Broadcast message to local subscribers of a document."""
        for uid in self.document_subscribers.get(document_id, set()):
            ...
            if ws:
                try:
                    ...
                    logger.error(
                        f"Broadcast to document {document_id} failed for user {uid}: {e}"
                    )

    async def broadcast_to_document(
        self, document_id: int, message: dict, exclude_user_id: Optional[int] = None
    ) -> None:
        """Broadcast message to all subscribers of a document (local + via Redis)."""
        await self._local_broadcast_to_document(
            document_id, message, exclude_user_id=exclude_user_id
        )
        await redis_pubsub.publish(
            "ws:document_broadcast",
            {
