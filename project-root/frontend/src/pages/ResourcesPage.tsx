  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-gray-800">Ресурсы и загрузка команды</h2>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Общая загрузка команды по неделям</h3>
        <div className="space-y-2">
          {aggregateLoad.map((w) => (
            <div key={w.week} className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-500 w-16">{w.totalHours}ч</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
          <h3 className="text-sm font-semibold text-gray-700">Загрузка по сотрудникам</h3>
        </div>
              ...
              {team.map((member: TeamMemberWorkload) => (
                <tr key={member.id} className="border-b border-gray-50 hover:bg-gray-50">
                  <td className="py-2 pr-3 pl-4">
                    ...
                    <div className="text-xs text-gray-400">{member.month_active_hours}ч за 30дн</div>
                  </td>
                  <td className="py-2 pr-3 text-xs text-gray-500">{member.role}</td>
                  <td className="py-2 pr-3 text-xs text-gray-600">{member.active_projects}</td>
                  <td className="py-2 pr-3 text-xs text-gray-600">{member.documents_total}</td>
                  ...
                  {member.weekly_load.map((wl: WeeklyLoad, i: number) => (
                    <td key={i} className="py-2 pr-3">
                      ...
                      <div className="text-[10px] text-gray-400 mt-0.5">{wl.hours} / {wl.capacity}ч</div>
                    </td>
                  ))}
                </tr>
              ))}
      ...
      {/* Active projects */}
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Активные проекты и занятость</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {active_projects.map((p) => (
            <div key={p.id} className="border border-gray-200 rounded p-3">
              ...
              <div className="text-xs text-gray-500 mt-1">👥 {p.team_size} человек</div>
            </div>
          ))}
        </div>
      </div>
